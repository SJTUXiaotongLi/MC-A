...
TEST_CASE("SquareLatticeIsingSystem 1") {
	const int dim = 2;
...
TEST_CASE("SquareLatticeIsingSystem Observables Brute-Force Counting") {
	...
	SquareLatticeDataBundle data_bundle(n_sites, beta);
...
//TEST_CASE("SquareLatticeIsingSystem Weiss field") {
//	const int dim = 2;
//	std::vector<int> L(dim);
//	L[0] = 4;
//	L[1] = 3;
...
//TEST_CASE("SquareLatticeIsingSystem Metropolis probability") {
//	const int dim = 2;
//	std::vector<int> L(dim);
//	L[0] = 4;
//	L[1] = 3;
...
//TEST_CASE("SquareLatticeIsingSystem Metropolis update") {
//	const int dim = 2;
//	std::vector<int> L(dim);
//	L[0] = 4;
//	L[1] = 3;
...
//TEST_CASE("SquareLatticeIsingSystem Metropolis update (2)") {
//	const int dim = 2;
//	std::vector<int> L(dim);
//	L[0] = 4;
//	L[1] = 3;
//	std::vector<double> beta = { 0.5 };
//	SquareLatticeDataBundle data_bundle(beta);
//	SquareLatticeIsingSystem test_system(L, &data_bundle);
//
//	const int seed = 745099137;
//	RandomNumberGenerator mtwist(seed);
...
bool SquareLatticeIsingSystem_MC_stderr_validation(const int L_spec, const double b_spec, const int n_trials, const double dev_tol = 3.0) {
	...
	L[1] = L[0];
	const int n_sites = SquareLatticeIsingSystem::eval_n_spins(L);
	std::cout << "System size in SquareLatticeIsingSystem_MC_stderr_validation(...) ... " << L[0] << " x " << L[1] << std::endl;
	...
	for (int i_trial = 0; i_trial < n_trials; i_trial++) {
		...
		SquareLatticeIsingSystem test_system(L, data_bundle, mcs_thermalization, mcs_interval_btwn_bins);
		if (i_trial==0) {
			test_system.exact_count();
...
TEST_CASE("SquareLatticeIsingSystem : MC validation") {
	const int L = 4;
	...
	for (std::vector<double>::iterator itr = beta.begin(); itr != beta.end(); itr++) {
		REQUIRE(SquareLatticeIsingSystem_MC_stderr_validation(L, *itr, n_trials) == true );
	}
}
