...
{
	std::cout << "# MC simulation (honeycomb lattice)" << std::endl;
	if (argc <= 1) {
		std::cerr << "We need a parameter file (see sample_parameters/...)" << std::endl;
		exit(1);
	}
	HoneycombLatticeParameterBundle_MC parameter(argv[1], true);
	...
	HoneycombLatticeIsingSystem system(L, data_bundle, mcs_thermalization, mcs_interval_btwn_bins);
