#include <random>
#include <iostream>
#include "Ising_Honey.hpp"

int main(int argc, char **argv)
{
	std::cout << "# MC simulation (Honey lattice)" << std::endl;
	if (argc <= 1) {
		std::cerr << "We need a parameter file (see sample_parameters/...)" << std::endl;
		exit(1);
	}
	HoneyLatticeParameterBundle_MC parameter(argv[1], true);
	
	/* Honey lattice system */
	const int dim = 2;
	...
	HoneyLatticeIsingSystem system(L, data_bundle, mcs_thermalization, mcs_interval_btwn_bins);
