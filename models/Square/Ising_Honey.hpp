#ifndef Ising_Honey_HPP
#define Ising_Honey_HPP

#include "Utils/IsingSystem.hpp"
#include "Utils/xml-parser/ParameterBundle.hpp"

class HoneyLatticeParameterBundle_exact : public ParameterBundle {
public:
	HoneyLatticeParameterBundle_exact(std::string input_file, bool print_option_spec = false) : ParameterBundle(print_option_spec) {
		init(input_file);
	};
...
class HoneyLatticeParameterBundle_MC : public ParameterBundle {
public:
	HoneyLatticeParameterBundle_MC(std::string input_file, bool print_option_spec = false) : ParameterBundle(print_option_spec) {
		init(input_file);
	};
...
class HoneyLattice : public Lattice {
private:
	static const int N_SL_HnLatt = 2;
	static const int z_common = 3;
	// static const int z_common_half = z_common / 2;
	static constexpr int z_HnLatt[N_SL_HnLatt] = { z_common, z_common};

public:
	HoneyLattice(const std::vector<int>& L_spec)
	: Lattice(N_SL_HnLatt, z_HnLatt, L_spec) {
		setup_NN_network();
	};
	...
	int eval_NN_site_index(int sl_index, const std::vector<int>& r, int bond_index) {
		...
		switch (bond_index) {
			...
			case 2:
				...
				break;
			default:
				std::cerr << "HoneyLattice::eval_NN_site_index> error" << std::endl;
				exit(1);
		}
...
class HoneyLatticeDataBundle : public DataBundle {
public:
	HoneyLatticeDataBundle(int n_spins_spec, const std::vector<double>& beta_spec, int n_bins_spec = 0, int n_samples_per_bin_spec = 0) : DataBundle(n_spins_spec, beta_spec, n_bins_spec, n_samples_per_bin_spec) {};
	
	~HoneyLatticeDataBundle() {};
...
class HoneyLatticeIsingSystem : public LatticeIsingSystem<HoneyLattice, HoneyLatticeDataBundle> {
public:
	HoneyLatticeIsingSystem(const std::vector<int>& L_spec, HoneyLatticeDataBundle& MCDB_spec, const int mcs_thermalization_spec = 0, const int mcs_interval_btwn_bins_spec = 0)
	: LatticeIsingSystem<HoneyLattice, HoneyLatticeDataBundle>(L_spec, eval_n_spins(L_spec), MCDB_spec, mcs_thermalization_spec, mcs_interval_btwn_bins_spec) {};

	HoneyLatticeIsingSystem(const std::vector<int>& L_spec) : LatticeIsingSystem<HoneyLattice, HoneyLatticeDataBundle>(L_spec, eval_n_spins(L_spec)){};
	
	~HoneyLatticeIsingSystem() {};
	
	double eval_energy() const {
		double energy_tmp = 0;
		const int z_common =  lattice._z_common();
		for (int site_idx = 0; site_idx < lattice._N_sites(); site_idx+=2) {
			for (int bond_index = 0; bond_index < z_common; bond_index++) {
