...
int main(int argc, char **argv) {
	std::cout << "# Exact evaluation (square lattice)" << std::endl;
	if (argc <= 1) {
		std::cerr << "We need a parameter file (see sample_parameters/...)" << std::endl;
		exit(1);
	}
	SquareLatticeParameterBundle_exact parameter(argv[1], true);
	...
	SquareLatticeIsingSystem system(L, data_bundle);
	system.exact_count();
