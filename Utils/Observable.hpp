...
public:
	MonteCarloObservable(int n_bins_spec, int n_samples_per_bin_spec) : n_bins(n_bins_spec), n_samples_per_bin(n_samples_per_bin_spec), current_bin_index(0), stderr_q(0), stderr_q_abs(0), stderr_q_sq(0), stderr_q_quar(0), stderr_q_fluc(0.0) {
...
public:
	...
	void output_legends_MC(const std::string system_size) {
		...
		std::cout << "# T[1] E[2] err[3] C[4] err[5] M[6] err[7] M^2[8] err[9] M^4[a] err[b]";
	};
